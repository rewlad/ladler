...
case class CheckBox(id:VDomKey,checked:Boolean)(val onCheck:Option[(VDomKey,Boolean)=>Unit]) extends
  VDomValue with OnCheckReceiver{
  var labelPosition="left"
  def appendJson(builder: JsonBuilder)={
    builder.startObject()
      builder.append("tp").append("Checkbox")
      builder.append("labelPosition").append(labelPosition)
...
class MaterialTags(
  ...
  def checkBox(key:VDomKey,id:VDomKey,checked:Boolean=false,check:(VDomKey,Boolean)=>Unit)=
    child[OfDiv](key,CheckBox(id,checked)(Some((id,newVal)=>check(id,newVal))),Nil)
